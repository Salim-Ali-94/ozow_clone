                ...
                <ContinueButton active={amount && (parseFloat(amount) > 0) && number && (number.length === 10) && reference ? true : false}
                                pressAction={() => { 
                                                    ...
                                                    const uuid = utility.uuid(10);
                                                     ...
                                                     setUser({ ...user, balance: user.balance - parseFloat(amount).toFixed(2), transactions: [{ direction: "from", reference: reference, category: constants.transactionCategories.filter(element => element.value === category)[0].label.toLowerCase().replace(" ", "_"), amount: parseFloat(parseFloat(amount).toFixed(2)), date: formattedDateTime, status: status, id: uuid }, ...user.transactions] });
                                                    //  axios.patch(DB_ENDPOINT + "updateBalance", { id: user.id, balance: user.balance - parseFloat(amount) });
                                                     ...
                                                     navigation.navigate("Confirmation", { animation: require("../../assets/animations/send.json"),
